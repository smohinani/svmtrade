...
Dependencies: numpy, pandas, scipy, requests, python-dotenv
"""
...
if not TRADIER_TOKEN:
    raise ValueError("TRADIER_TOKEN not found in environment variables")

# Local cleaning function (avoid circular import)
def clean_tradier_data(raw_data: pd.DataFrame) -> pd.DataFrame:
    if raw_data.empty:
        raise ValueError("Downloaded data is empty.")

    # Tradier data should already have proper column names
    required_cols = ['open', 'high', 'low', 'close', 'volume']
    if not all(col in raw_data.columns for col in required_cols):
        raise KeyError(f"Missing required columns. Available: {list(raw_data.columns)}")
